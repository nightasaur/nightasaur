...
限制：已完成認證並在執行中的請求不會倒退取消。現有未登錄 sessions 的舊 JWT 將失效，需要重新登入；不改帳號密碼。現有 Session schema 可儲存雜湊，毋須新增欄位；本次不執行正式 DB 操作。登入會清除該帳號過期記錄並保留最多 20 個工作階段；多副本登入限速仍需共用或邊緣層實作。
