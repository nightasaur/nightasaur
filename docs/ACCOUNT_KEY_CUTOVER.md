...
The helper is not exposed as an HTTP route or automatic migration. Its test uses disposable SQLite and fixture users. Production execution, identity verification, secret-manager provisioning and runtime acceptance remain operator actions. The helper does not replace rate limiting and incident response. The updated authentication runtime separately requires persisted session digests; logout removes its matching session. Legacy runtimes do not enforce this check. On rollout, existing JWTs without persisted matching digests require a new login; passwords are unchanged. See SESSION_MODEL.md.
