# CREDITS — declarations require review

The historical tables below are unverified attribution leads, not permission to copy or commercial clearance. Consult [the versioned inventory](docs/PROVENANCE.md). In particular, GPL obligations are not satisfied merely by credit; model restrictions and proprietary assets require separate review. No third-party work inherits this repository's MIT license.


## Special Thanks

Nightasaur stands on the shoulders of giants. We gratefully acknowledge the following open-source projects, frameworks, and tools that made this project possible.

---

## 🧠 Language Models (AI/ML)

| Name | License | Usage | Can Copy? |
|------|---------|-------|-----------|
| [Ollama](https://ollama.com) | **MIT** ✅ | Local LLM runner | ✅ Yes |
| Text inference model | No model selected or approved by this change | Disabled by default; project-prohibited model family remains blocked | Requires exact-version provenance and license review |
| [Stable Diffusion SD 1.5](https://huggingface.co/runwayml/stable-diffusion-v1-5) | **CreativeML Open RAIL-M** ⚠️ | Image generation base model | ⚠️ Attribution required |
| [ComfyUI](https://github.com/comfyanonymous/ComfyUI) | **GPL 3.0** ⚠️ | AI image generation pipeline | ⚠️ Must credit |

---

## 📦 Core Dependencies

### Node.js (Backend)

| Package | License | Can Copy? |
|---------|---------|-----------|
| [Express](https://expressjs.com) | **MIT** ✅ | ✅ |
| [Prisma](https://prisma.io) | **Apache 2.0** ✅ | ✅ |
| [bcryptjs](https://github.com/dcodeIO/bcrypt.js) | **MIT** ✅ | ✅ |
| [jsonwebtoken](https://github.com/auth0/node-jsonwebtoken) | **MIT** ✅ | ✅ |
| [cors](https://github.com/expressjs/cors) | **MIT** ✅ | ✅ |
| [multer](https://github.com/expressjs/multer) | **MIT** ✅ | ✅ |
| [axios](https://axios-http.com) | **MIT** ✅ | ✅ |
| [zod](https://zod.dev) | **MIT** ✅ | ✅ |
| [uuid](https://github.com/uuidjs/uuid) | **MIT** ✅ | ✅ |

### Frontend

| Package | License | Can Copy? |
|---------|---------|-----------|
| [React](https://react.dev) | **MIT** ✅ | ✅ |
| [React Router](https://reactrouter.com) | **MIT** ✅ | ✅ |
| [Vite](https://vitejs.dev) | **MIT** ✅ | ✅ |
| [Tailwind CSS](https://tailwindcss.com) | **MIT** ✅ | ✅ |
| [TypeScript](https://typescriptlang.org) | **Apache 2.0** ✅ | ✅ |
| [Axios](https://axios-http.com) | **MIT** ✅ | ✅ |
| [framer-motion](https://framer.com/motion) | **MIT** ✅ | ✅ |

### Python (AI Engine)

| Package | License | Can Copy? |
|---------|---------|-----------|
| [FastAPI](https://fastapi.tiangolo.com) | **MIT** ✅ | ✅ |
| [uvicorn](https://www.uvicorn.org) | **BSD** ✅ | ✅ |
| [httpx](https://www.python-httpx.org) | **BSD** ✅ | ✅ |
| [pydantic](https://docs.pydantic.dev) | **MIT** ✅ | ✅ |
| [Pillow](https://python-pillow.org) | **Historical Permission Notice** ✅ | ✅ |

---

## 🏗️ Infrastructure & Tools

| Tool | License | Can Copy? |
|------|---------|-----------|
| [PostgreSQL](https://postgresql.org) | **PostgreSQL License** ✅ | ✅ |
| [Redis](https://redis.io) | **BSD** ✅ | ✅ |
| [Docker](https://docker.com) | **Apache 2.0** ✅ | ✅ |
| [Node.js](https://nodejs.org) | **MIT** ✅ | ✅ |
| [Vercel](https://vercel.com) | Platform (free tier) | N/A |
| [Railway](https://railway.app) | Platform (free tier) | N/A |

---

## 🎨 Assets & Design

| Asset | License | Can Copy? |
|-------|---------|-----------|
| All Nightasaur spirit images | **Proprietary** ⚠️ | ❌ (Original art) |
| Emoji icons (Unicode) | **Open Source** ✅ | ✅ |
| Tailwind UI patterns | **MIT** ✅ | ✅ |

---

## 📚 Reference Projects & Inspiration

These projects were studied for **architectural inspiration only** — no code was directly copied:

| Project | License | Notes |
|---------|---------|-------|
| [ChatGPT](https://chat.openai.com) | Proprietary | Design inspiration |
| [Pokémon](https://pokemon.com) | © Nintendo | Conceptual inspiration (game mechanics) |
| [Digimon](https://digimon.net) | © Bandai | Conceptual inspiration (evolution system) |

---

## 📝 License Compatibility Notes

| Nightasaur License | **MIT** |
|--------------------|---------|
| Can be used in proprietary projects? | ✅ Yes |
| Can be used in other MIT projects? | ✅ Yes |
| Can be used in GPL projects? | ✅ Yes (one-way compatible) |
| Can include code from MIT projects? | ✅ Yes (with attribution) |
| Can include code from Apache 2.0 projects? | ✅ Yes (with attribution) |
| Can include code from GPL projects? | ⚠️ **Be careful** — may require GPL |
| Can include code from proprietary projects? | ❌ **No** |

---

## 📞 Questions?

If you have questions about the licensing of any specific component, please open an issue on GitHub.

---

**Made with ❤️ by the Nightasaur Team | 2026**
